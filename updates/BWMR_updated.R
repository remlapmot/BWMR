...
ELBO_func <- function(N, gammahat, Gammahat, sqsigmaX, sqsigmaY, mu_beta, sqsigma_beta, mu_gamma, sqsigma_gamma, a, b, pi_w, sqsigma, sqtau, alpha=100, sqsigma0 = (1e+6)^2) {
...
BWMR <- function(gammahat, Gammahat, sigmaX, sigmaY, alpha=100, sqsigma0 = (1e+6)^2) {
  ## data
  N <- length(gammahat)
  sqsigmaX <- sigmaX^2
  sqsigmaY <- sigmaY^2
  ...
  plot4 <- ggplot2::ggplot(df4, ggplot2::aes(x=gammahat, y=Gammahat, color=w)) + ggplot2::geom_point(size = 0.3) +
    ...
    ggplot2::geom_abline(intercept=0, slope=mu_beta, color="#990000", linetype="dashed", size=0.5) +
